...
OPTIONS:
	...
	-h			Make hydrogens polarisable, default $PolHydrogens
	-n			name of series
...
if [[ $STARTPARS == 'components' ]]; then
	if [[ $JMType == 'TholeLinAniso' ]]; then echo -en "cenpos={'H':0.5974, 'C':7.7764, 'Si':7.7764, 'N': 4.7575, 'O': 2.6442, 'F': 0.8149, 'S': 11.0208, 'Se': 11.0208, 'Cl':7.5682, 'Br': 12.7339, 'I':20.2867, 'screen':1.6623}\n" >> outputfile.py
	elif [[ $JMType == 'TholeExp' ]]; then echo -en "cenpos={'H':1.3849, 'C':6.5780, 'Si':6.5780, 'N': 2.9855, 'O': 2.3184, 'F': 1.4906, 'S': 10.8651, 'Se': 10.8651, 'Cl':6.7129, 'Br': 12.3384, 'I':20.0081, 'screen':2.8990}\n" >> outputfile.py
	fi
fi
...
if [[ $STARTPARS == 'empirical' ]]; then
	...
	elif [[ $JMType == 'TholeExp' ]]; then echo -en "cenpos={'H':2.7927, 'C':8.6959, 'Si':8.6959, 'N': 6.5565, 'O': 5.7494, 'F': 3.0013, 'S': 16.6984, 'Se': 19.7422,  'Cl':16.1979, 'Br': 23.5714, 'I':36.9880, 'screen':2.1304}\n" >> outputfile.py
	fi
fi
...
do
 ...
 do
  ...
  if [[ $(grep -c "elname'\:'$atomtype'" ${file}.xyz) -ge 1 && $(echo "$etaopts" | grep -c "${atomtype}=${atomtype}") == "0" ]]
  then
...
if [[ $POLFILE == "" ]]
then
	POLFILE="$( echo ${file} | sed 's/_aniso_w_connectivity//')"
fi

POLFILE=$(echo "$POLFILE" | sed 's/.py//g')

echo -en "from Molecules.${POLFILE} import pol\nHFpol_${file}=pol\n" >> outputfile.py
done
...
do
  ...
  file=$(echo "$file" | sed 's/.xyz//g')
  echo -en "${FitType}('../Molecules/${file}.xyz',HFpol_${file}$etaopts,jmtype='$JMType',screenradius=screen)" >> outputfile.py
done
...
then
	...
	cp outputfile.py ../Singlemol_Pol_Determination/${basename}fixscreen_multipolfit.py
	cat << EOF >> ../Singlemol_Pol_Determination/${basename}fixscreen_multipolfit.py
...
EOF
	...
	cp outputfile.py ../Singlemol_Pol_Determination/${basename}fixscreen_multipolfit_wscan.py
	cat << EOF >> ../Singlemol_Pol_Determination/${basename}fixscreen_multipolfit_wscan.py
...
EOF
	...
	cp outputfile.py ../Singlemol_Pol_Determination/${basename}fitscreen_multipolfit.py
	cat << EOF >> ../Singlemol_Pol_Determination/${basename}fitscreen_multipolfit.py
...
EOF
	...
	#make version without fitscreen, with scan
	cp outputfile.py ../Singlemol_Pol_Determination/${basename}fitscreen_multipolfit_wscan.py
	cat << EOF >> ../Singlemol_Pol_Determination/${basename}fitscreen_multipolfit_wscan.py
