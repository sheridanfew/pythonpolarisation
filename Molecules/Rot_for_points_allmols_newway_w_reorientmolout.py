...
atomsforlongaxis={"TIPS_Pc": 12, "C8_BTBT": 3, "sexithiophene": 5, "Rubrene": 4, "Pc":1, "PTCDA":12, "PDIFCN2": 53, "TMTSF": 3}
perpatoms={"TIPS_Pc": 7, "C8_BTBT": 16, "sexithiophene": 28, "Rubrene": 39, "Pc":11, "PTCDA":14,"PDIFCN2": 55, "TMTSF": 0}
nmolsincell={"TIPS_Pc": 1, "C8_BTBT": 2, "sexithiophene": 4, "Rubrene": 4, "Pc":2, "PTCDA":1,"PDIFCN2": 1, "TMTSF": 1}

for molecule in ['TIPS_Pc']:
#for molecule in ['Pc','C8_BTBT','sexithiophene','TIPS_Pc','Rubrene','PTCDA','PDIF_CN2','TMTSF']:
	print molecule
	for molincell in ['a','b','c','d']:
		nameout_coords=str(molecule + '_mol' + molincell + '_opt_reorient.xyz')
		print nameout_coords
		for charge in ['neut']:
			Importmol_cif=None
			for prefix in ['old/','oldway/','']:
				#if os.path.isfile(prefix + molecule + '_mol' + molincell + '.xyz'):
				#	Importmol_cif=(prefix + molecule + '_mol' + molincell + '.xyz')
				if os.path.isfile(prefix + molecule + '_mol' + molincell + '_' + charge + '_aniso_cifstruct_mul.xyz'):
					Importmol_cif=(prefix + molecule + '_mol' + molincell + '_' + charge + '_aniso_cifstruct_mul.xyz')
					nameout=str('sp_' + molecule + '_mol' + molincell + '_' + charge + '.xyz')
				if os.path.isfile(prefix + molecule + '_mol' + molincell + '_' + charge + '_fit_aniso_cifstruct_mul.xyz'):
					Importmol_cif=(prefix + molecule + '_mol' + molincell + '_' + charge + '_fit_aniso_cifstruct_mul.xyz')
					nameout=str('sp_' + molecule + '_mol' + molincell + '_' + charge + '.xyz')
				if (nmolsincell[molecule] == 1) and (os.path.isfile(molecule + '_' + charge + '_aniso_cifstruct_mul.xyz')):
					Importmol_cif=(prefix + molecule + '_' + charge + '_aniso_cifstruct_mul.xyz')
				...
				if (nmolsincell[molecule] == 1) and (os.path.isfile(molecule + '_' + charge + '_fit_aniso_cifstruct_mul.xyz')):
					Importmol_cif=(prefix + molecule + '_' + charge + '_fit_aniso_cifstruct_mul.xyz')
					nameout=str('sp_' + molecule + '_' + charge + '.xyz')
				if os.path.isfile(prefix + 'py_' + molecule + '_mol' + molincell + '_cif.xyz'):
					Importmol_cif=(prefix + 'py_' + molecule + '_mol' + molincell + '_cif.xyz')
					nameout=str('sp_' + molecule + '_mol' + molincell + '_' + charge + '.xyz')

			print 'Importmol_cif ', Importmol_cif

			if Importmol_cif:
				print Importmol_cif
				if (os.path.isfile('py_' + molecule + '_opt.xyz')):
					Importmol_gauss=('py_' + molecule + '_opt.xyz')
				elif (os.path.isfile(molecule + '_' + charge + '_fit_aniso_mul.xyz')):
					Importmol_gauss=(molecule + '_' + charge + '_fit_aniso_mul.xyz')
				elif (os.path.isfile('old/' + molecule + '_' + charge + '_aniso_mul.xyz')):
					Importmol_gauss=('old/' + molecule + '_' + charge + '_aniso_mul.xyz')

				print 'Importmol_gauss', Importmol_gauss
				...
				for atom in Mol_gauss():
					#print atom()._elname
					atomtypes.append(np.array(atom()._elname))
				...
				#Put CoM of Gauss Molecule in position of CoM of .cif molecule. This is cheifly for calcs of overlap integral, which are better performed with optimised structures put in pos'ns as .cif.

				posns_opt_reorient=[]
				for i in posns_gauss_rot:
					#print 'i', i
					posns_opt_reorient.append(0.5291772109217*(i+centre_cif))
					#print 'i+centre_cif',i+centre_cif
					#print 'i[0,0]', i[0,0]

				stdout=sys.stdout
				sys.stdout = open(nameout_coords, 'w')
				for pos,atomtype in zip(posns_opt_reorient, atomtypes):
					if atomtype != 'single' and  atomtype != 'pi' and  atomtype != 'double'  and  atomtype != 'dpi'  and  atomtype != 'triple':
						print atomtype, " ", pos[0,0], " ", pos[0,1], " ", pos[0,2]
				sys.stdout = stdout
