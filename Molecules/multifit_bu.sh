...
while getopts "abd:f:hj:n:s:?" Option
do
    case $Option in
	a	  ) FitScreen="TRUE";;
	b	  ) NOBONDS="TRUE";;
...
do
 ...
 do
  ...
  if [[ $(grep -c "elname'\:'$atomtype'" ${file}_fit_aniso_no_charge.xyz) -ge 1 && $(echo "$etaopts" | grep -c "${atomtype}=${atomtype}") == "0" ]]
  then
...
do
  ...
  echo -en "${FitType}('../Molecules/${file}_fit_aniso_no_charge.xyz',HFpol_${file}$etaopts,jmtype='$JMType',screenradius=screen)" >> outputfile.py
done
