...
rotrange=np.arange(30.0, 45.0, 30.0)

cut=1.1

f = open('../../../Blends/PTcat_PCBM_EDITEDO_anion_mul/8T_PCBMtest_2013_03_14/8T_PCBMtest_2013_03_14.csv', 'w')
...
#csv file
			...
			f = open('../../../Blends/PTcat_PCBM_EDITEDO_anion_mul/8T_PCBMtest_2013_03_14/8T_PCBMtest_2013_03_14.csv', 'a')
			f.write ('\n8Tcatmul.xyz:PCBM_EDITEDO_anion_mul\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s' % (regionnames[i],rotangle,distA,energyev,Uqq,Uqd,Udd,tot))
			f.flush()
			f.close()

#.dat for gnuplot

			f = open('8Tcatmul.xyz_PCBM_EDITEDO_anion_mul_%s_%gdeg_%gA.dat' % (regionnames[i],rotangle,distA), 'w')
