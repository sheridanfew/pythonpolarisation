# 8Tcation:PCBM_EDITEDO_anion_mul polarisation model. from files:
#8T_cation_B3LYP_6-31gSTAR_opt_HF_6-31gSTAR.log
...
ReadMoleculeType('../../../Molecules/./8T_cation_B3LYP_6-31gSTAR_opt_HF_6-31gSTAR.log')
don = GetMolecule('../../../Molecules/./8T_cation_B3LYP_6-31gSTAR_opt_HF_6-31gSTAR.log')
donatomlist=[]
for atom in don():
...
f = open('../../../Blends/PT_PCBM_EDITEDO_anion_mul/8T_PCBMtest_2013_05_29/8T_PCBMtest_2013_05_29.csv', 'w')
f.write ("System\tAlignment\tAngle\tDistance\tEnergy\tUqq\tUqd\tUdd\tDipole Moment")
f.flush()
f.close()

regionnames=['4th_ring']
atomnos=np.array([[22, 23, 24, 25, 26, 27, 28]])
...
#csv file
			...
			f = open('../../../Blends/PT_PCBM_EDITEDO_anion_mul/8T_PCBMtest_2013_05_29/8T_PCBMtest_2013_05_29.csv', 'a')
			f.write ('\n8Tcation:PCBM_EDITEDO_anion_mul\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s' % (regionnames[i],rotangle,distA,energyev,Uqq,Uqd,Udd,tot))
			f.flush()
			f.close()

#.dat for gnuplot

			f = open('8Tcation_PCBM_EDITEDO_anion_mul_%s_%gdeg_%gA.dat' % (regionnames[i],rotangle,distA), 'w')
