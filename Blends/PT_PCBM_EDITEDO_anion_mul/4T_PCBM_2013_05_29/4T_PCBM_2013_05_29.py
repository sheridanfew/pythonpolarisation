...
#get rotation matrix to this versor from z (may not be most efficient way. Also, psi taken as 0, affects rotational orientation of fullerene face aliogned w. polymer, don't think important)
	...
	for rotangle in rotrange:
		print 'type perp, rotangle tanangle'
		tp=type(perp[i][0])
		tr=type(rotangle)
#		tat=type(((perp[i][0]*perp[i][0])+(perp[i][1]*perp[i][1])))
#		print tp, tr, tat
		if perp[i][2] == 1:
...
#csv file
			...
			f = open('../../../Blends/PT_PCBM_EDITEDO_anion_mul/4T_PCBM_2013_05_29/4T_PCBM_2013_05_29.csv', 'a')
			f.write ('\n4TCONSTRAINED:PCBM_EDITEDO_anion_mul\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s' % (regionnames[i],rotangle,distA,energyev,Uqq,Uqd,Udd,tot))
			f.flush()
			f.close()

#.dat for gnuplot

			f = open('4TCONSTRAINED_PCBM_EDITEDO_anion_mul_%s_%gdeg_%gA.dat' % (regionnames[i],rotangle,distA), 'w')
