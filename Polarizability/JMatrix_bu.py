...
class JMatrix(object):
	...
	def __init__(self, **kwargs):
		...
		elif self._jmtype == 'TholeLinAniso' or self._jmtype == 'TholeLinIso': #See Duijnen et al. Table 7 for these values
			if kwargs.get('screenradius'):
	...
	def get_element(self,atom1, atom2):
		...
		if atom1 is atom2 :
			...
			if self._jmtype == 'Stern':
				...
				Tdiags= 1./d**3 * I * ( 1 - ( exp(-self._screenradius*d ) * (1 + self._screenradius * d + self._screenradius**2*d**2/2 ) ) ) 
				Toffdiags= - 3. * (dV.T * dV )/d**5 * ( 1 - ( exp(-self._screenradius*d ) * (1 + self._screenradius * d + self._screenradius**2*d**2/2 + self._screenradius**3*d**3/6 ) ) )
				T = Tdiags + Toffdiags
				return T

			elif self._jmtype == 'TholeLinIso':
				s=self._screenradius*((atom1._pol[0,0]*atom2._pol[0,0])**(1/6))
				v= d/s
				if d >= s:
			...
			elif self._jmtype == 'TholeLinAniso':
