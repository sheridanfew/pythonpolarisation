...
ReadMoleculeType('../Molecules/./TEST_proton_pos100.xyz')
ReadMoleculeType('../Molecules/./TEST_proton_pos100.xyz')

ReadMoleculeType('../Molecules/./TEST_neutron_pos000.xyz')
ReadMoleculeType('../Molecules/./TEST_neutron_pos100.xyz')

centre='TEST_proton_pos100.xyz'
surroundings='TEST_proton_pos100.xyz'
...
for size in np.arange(1,2,1):
	...
	#Efield_mat = [[[] for i in range((2*size)+1)] for i in range((2*size)+1)]
	#Efield_mat is a matrix, each element of which contains arrays of external E-fields acting on each polarisable dipole, for a charge placed at the position  labelled by the element of E-field_mat. (Note that this does not contain E field due to dipoles). This is used to calc reorg energy.
	print mols
		...
		for y in np.arange(-size,size+1,1):
			...
			for z in np.arange(-size,size+1,1):
				...
				if x == 0 and y == 0 and z == 0:
					#central mol
					mols[xlist][ylist].append(GetMolecule('../Molecules/./TEST_proton_pos100.xyz'))
					...
					mols[xlist][ylist].append(GetMolecule('../Molecules/./TEST_neutron_pos100.xyz'))
					#print mols
