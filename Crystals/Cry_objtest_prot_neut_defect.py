...
#prot_neut_cry=Crystal(name=name,mols_cen=mols_cen,mols_sur=mols_sur,TVs=TV,cenpos=[1,1,1],lena=2,lenb=2,lenc=2)
prot_neut_cry_top=Crystal(name=name,mols_cen=mols_cen,mols_sur=mols_sur,cenpos=[1,0,1],lena=2,lenb=2,lenc=2,TVs=TV,maxTVs=1)
prot_neut_cry_bot=Crystal(name=name,mols_cen=mols_cen,mols_sur=mols_sur,cenpos=[2,1,1],lena=2,lenb=2,lenc=2,TVs=TV,maxTVs=1)

#=> centred in bc on top, centred in ac on bottom => rotate ac onto bc), rotate by alpha about TV[0] (a) => new cry above in a

rotmat=Rotation.Rotation(rodrigues=[TV[2][0],TV[2][1],TV[2][2],gamma)
...
# print dipoles
f = open('%s_size_%s_dipoles.dat' % (name,(lena+1)*(lenb+1)*(lenc+1)), 'w')
for dd in split_d:
	dstr=str(dd)
	f.write(dstr)
	f.write('\n')
