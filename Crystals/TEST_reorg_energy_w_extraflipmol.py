...
ReadMoleculeType('../Molecules/./TEST_pointdipx_pos111.xyz')
ReadMoleculeType('../Molecules/./TEST_pointdipx_pos111.xyz')

centre='TEST_pointdipx_pos111.xyz'
surroundings='TEST_pointdipx_pos111.xyz'
...
#Converts frac coords to carts, see http://en.wikipedia.org/wiki/Fractional_coordinates
		...
		for y in np.arange(-size,size+1,1):
			...
			for z in np.arange(-size,size+1,1):
				...
				if x == 0 and y == 0 and z == 0:
					#central mol
					mols[xlist][ylist].append(GetMolecule('../Molecules/./TEST_pointdipx_pos111.xyz'))
					print 'xlist,ylist,zlist',xlist,ylist,zlist
					#flippedmol
					molsflip[xlist][ylist].append(GetMolecule('../Molecules/./TEST_pointdipx_pos111.xyz'))
					#print mols
					for atom in molsflip[xlist][ylist][zlist]():
						...
						print 'atom_pol_diags_flipped=-(atom_pol_diags-(2*Dpol*n_refl_norm)).T'
						print 'atom_pol_diags_flipped=-(', atom_pol_diags,' - (2*',Dpol,'*',n_refl_norm,')).T'

						print 'magnitude of n_refl_norm',np.dot(n_refl_norm,n_refl_norm.T)
					...
					for atom in molsflip[xlist][ylist][zlist]():
						#Flip molecule coords
						D=(np.dot(n_refl,atom()._pos.T))+dist_refl_norm
						movevec=2*D*n_refl
						atom().move(movevec)
						#Flip polarisabilities
						Dpol=(np.dot(n_refl,atom()._pol.T))
						atom()._pol=atom()._pol-(2*D*n_refl)
						#Translate to correct cell
						atom().move(x*TV[0]+y*TV[1]+z*TV[2])
						print 'flipped'
#atom pos:', atom()._pos
